...
######################################################
SPARForestERFlinear <- function(Y, e, x, Z, W, epsilon, max.iter, ntrees, mtry, minnode, n.sample, ntrees.sample)
{
#########################
...
##################
   ...
   formula.spatial <- Y ~ offset(log(e)) + x + 
   ...
   ERF.new <- dat.spatial$x * mod.spatial$summary.fixed$mean[2]
   ...
   ERFandRE.samples <- array(NA, c(n.sample, K))
      ...
      sample.confounder <- samples[[j]]$latent[(3*K+1):(4*K)]
      sample.intercept <- samples[[j]]$latent[(4*K+1)]
...
ERF.estimate <- data.frame(exposure=dat.spatial$x, 
                      estimate=dat.spatial$x * mod.spatial$summary.fixed$mean[2], 
                      LCI=dat.spatial$x * mod.spatial$summary.fixed$`0.025quant`[2],
                      UCI=dat.spatial$x * mod.spatial$summary.fixed$`0.975quant`[2])
