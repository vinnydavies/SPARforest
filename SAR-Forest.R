########### SARforest function  ################
SARForest <- function(X.training, X.test, Y.training, coords.training, coords.test, n_tr, m_try, min_node, D, R, logscale, inla_samples)
{
  ####################
  #### Specify lengths
  ####################
  ...
  for(r in 1:R)
  {
    ...
    formula.sar <-  response ~ offset(offset) +  f(region, model="slm",
                                                   args.slm = list(rho.min=0, 
                                                                   rho.max=1,
                                                                   X=matrix(0, K.total, 0),
                                                                   W = Wstd,
                                                                   Q.beta=Diagonal(n=0, 0.00001)),
                                                   hyper=list(rho=list(prior="gaussian", param=c(0, 0.01)), 
                                                              prec=list(prior = "loggamma", param = c(1, 0.01))))
    mod.sar <- inla(formula=formula.sar, data=dat.all, family="gaussian",
    ...
    phi.training <- mod.sar$summary.random$region$mean[1:K.training]
    phi.test <- mod.sar$summary.random$region$mean[(K.training+1):K.total]
    
    
    #### Predict from the SAR model
    samples.parameters <- inla.posterior.sample(n=inla_samples, result=mod.sar, add.names=TRUE, use.improved.mean = TRUE, skew.corr=TRUE)
    ...
    if(logscale)
    {
      sigma2.mean <- 1 / mod.sar$summary.hyperpar[1,1]
